...
{
    global $link;
    $sql = mysqli_query($link, "INSERT INTO `posts` (`title`,`image`, `content`) VALUES ('{$_POST['title']}','{$_FILES['image']['name']}','{$_POST['content']}')");
    if ($sql) {
...
function update_post_metod(){
    ...
    $sql = mysqli_query($link,"update metod set title='{$_POST['title']}',text ='{$_POST['text']}',file='{$_FILES['file']['name']}' where `id` ='{$_GET['edit']}'");
    if ($sql) {
        echo '<p>Данные обновлены.</p>';
    } else {
        echo '<p>Произошла ошибка: ' . mysqli_error($link) . '</p>';
